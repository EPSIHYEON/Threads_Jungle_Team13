...
{
	// 바꼈는데, readylist 보다 우선순위가 작다면 yield 로 바꿈
	thread_current()->priority = new_priority;

	if (list_front(&ready_list) > new_priority) // 바꾼 priority 가 작으면 thread_yield
		thread_yield();

	// list_insert_ordered(&ready_list, &thread_current()->elem, compare_less,NULL);
}
...
{
	struct thread *t_a = list_entry(ele, struct thread, elem);

	struct thread *t_b = list_entry(e, struct thread, elem);
